...
class GraphSim(pl.LightningModule):
  ...
  def __init__(self, *args, **kwargs) -> None:
    ...
    # dynamics graph network (you must implement this or replace with your model)
    self.dynamics_gn = nets.EncodeProcessDecode(
        node_dim=self.latent_dim + (1 if self.hparams.use_weights else 0),
        edge_dim=self.hparams.type_emb_size + 4,  # example, adjust as needed
        out_dim=self.latent_dim,
        **self.hparams.dynamics_gn_hparams
    )
    
    # decoder to predict next position (you can customize)
    self.decoder = nets.build_mlp(in_dim=self.latent_dim, out_dim=3, hidden_dims=[128, 128])
  ...
  def predict(self, pos_seq, n_node, ptype_embeddings, bonds, weights, lattices=None, deterministic=False):
    """
    Given a sequence of positions and embeddings, predict next position and optionally properties.
    """
    graph_input = self._dynamics_preprocessor(pos_seq, ptype_embeddings, n_node, bonds, lattices, weights=weights)

    latent_graph = self.dynamics_gn(graph_input)

    next_pos_pred = self.decoder(latent_graph.nodes)
  ...
  def forward(self, *args, **kwargs):
    """
    Override this with the appropriate forward pass logic.
    """
  ...
  def step(self, batch, batch_idx: int):
    ...
    """
    raise NotImplementedError("Please implement the step method.")
  ...
  def wrap_batch(self, pos_seq, lattices, n_node):
    """
    Wrap batched coordinates in the lattices.
    """
  ...
  def _embedding_preprocessor(self, ptypes, n_node, bonds=None, btypes=None, weights=None):
    """
    Compute node embedding at the fine graph level. Execute before coarse-graining.
    ...
    if not self.hparams.embedding_gn_hparams:
      ...
      assert bonds is not None, 'an embedding network should only be used when the ground truth graph contains bonds.'                        
  ...
  def predict_prop(self, latent_graph, pos, weights, n_node, prop='rgs'):
    """
    Predict the specified property if applicable.
